...
class GameScene: SKScene {
    ...
    func addFloot() {
        ...
        let invisibleRoor = SKNode()
        invisibleRoor.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: size.width, height: 1))
        invisibleRoor.physicsBody?.isDynamic = false
        invisibleRoor.position = CGPoint(x: size.width / 2, y: size.height)
        invisibleRoor.zPosition = 2
        
        
        addChild(invisibleRoor)
    }
    ...
    func addIntro() {
        ...
        intro.position = CGPoint(x: self.scene!.size.width / 2, y: size.height - 210.0)
        intro.zPosition = 3
        addChild(intro)
    }
